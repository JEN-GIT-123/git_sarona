        ...
        {steps.map((s, i) => (
          <div key={s.number} className="flex-1 flex items-center">
            ...
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold
              ${step === s.number ? "bg-pink-500 text-white" : step > s.number ? "bg-pink-400 text-white" : "bg-pink-200 text-pink-600"}`}>
              {s.number}
            </div>
            {/* Title */}
            <div className="ml-2 text-sm font-semibold text-pink-700">{s.title}</div>
            {/* Line */}
            {i < steps.length - 1 && (
              <div className={`flex-1 h-1
                ${step > s.number ? "bg-pink-500" : "bg-pink-200"}`}>
              </div>
            )}
          </div>
        ))}
        ...
        <div className="max-w-lg mx-auto bg-white p-6 rounded-3xl shadow-lg space-y-4 border-2 border-pink-200">
          ...
          {["name","email","country","address","phone"].map(f => (
            <input
              ...
              className="w-full p-3 rounded-xl border-2 border-pink-200 focus:ring-2 focus:ring-pink-300 outline-none"
            />
          ))}
          <button
            onClick={handleNext}
            className="w-full mt-4 bg-pink-500 text-white rounded-full py-3 font-semibold hover:bg-pink-600 transition flex items-center justify-center gap-2"
          >
            <FaShoppingCart /> Next
          </button>
        </div>
        ...
        <div className="max-w-4xl mx-auto bg-white p-6 rounded-3xl shadow-lg space-y-6 border-2 border-pink-200">
          ...
          <div className="flex justify-between gap-4 mt-4">
            ...
            <button
              ...
              className="flex-1 py-3 bg-pink-500 text-white rounded-full font-semibold hover:bg-pink-600 transition flex items-center justify-center gap-2"
            >
              Payment 💖
            </button>
          </div>
            ...
            {["ACLEDA Bank","ABA Bank","Card"].map(m => (
              <button
                ...
                className={`w-full py-3 rounded-full font-semibold flex items-center justify-between px-4 transition
                  ${paymentMethod===m?"bg-pink-500 text-white":"bg-pink-100 text-pink-700 hover:bg-pink-200"}`}
              >
